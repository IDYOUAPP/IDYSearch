...
const handler = async (event) => {
      ...
      switch (eventName) {
        ...
        case "MODIFY":
          if (newImage) {
            ...
            const insertPayloadSendBird = InsertHelper.routeRequestToSendBirdHelper(newImage);
            if (insertPayload) algoliaItem.INSERT.push(insertPayload);
            if (insertPayloadSendBird) sendBirdItem.UPDATE.push(insertPayloadSendBird);
          }
          break;
        case "REMOVE":
          if (oldImage) {
  ...
  try {
    ...
    if (sendBirdItem.UPDATE.length > 0) {
      await sendBirdHelper.upsertRecords(sendBirdItem.UPDATE);
    }
